...
const DashboardPanelGithub = (props) => {
  ...
  const releasesLoading =
    appReleases?.loading || apiReleases?.loading || adminReleases?.loading;
  const hallOfFameLoading =
    appHallOfFame?.loading ||
    apiHallOfFame?.loading ||
    adminHallOfFame?.loading;
  ...
  const openCreateModal = (header, fields, endpoint, bodyConstructor) =>
    openModal({
      ...
      onConfirm: ({setLoading, setSuccess, setError, values}) => {
        setLoading();
        let body =
          bodyConstructor === null ? {...values} : bodyConstructor({...values});
        post(endpoint, body)
          .then(() => {
      ...
      {user?.isAdminDeveloper ? (
        <Box float="center">
          ...
          <Button
            ...
            onClick={() =>
              openCreateModal(
                ...
                runStagingMigration,
                runStagingMigrationEndpoint,
                createStagingMigrationBody
              )
            }
          >
            Run Staging Migration
          ...
          <Button
            ...
            onClick={() =>
              openCreateModal(
                ...
                runProductionMigration,
                runProductionMigrationEndpoint,
                createProductionMigrationBody
              )
            }
          >
            Run Production Migration
          ...
          <Button
            ...
            onClick={() =>
              openCreateModal(
                'Click Save Changes to Run Production DB Replication',
                null,
                runProductionsDBReplicationEndpoint,
                createProductionDBReplicationBody
              )
            }
          >
                  ...
                  actions={[
                    {
                      label: 'Profile',
                      onClick: (profile) =>
                        openRelease(profile.profile_username_url),
                    },
                  ]}
              ...
              {appReleases?.data?.data?.length > 0 ? (
                <Table
                  ...
                  actions={[
                    {
                      label: 'View Release',
                      onClick: (release) => openRelease(release.release_url),
                    },
                  ]}
