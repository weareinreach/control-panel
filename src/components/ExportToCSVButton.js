...
const ExportToCSVButton = ({ margin = '0px', query }) => {
  ...
  const handleCSV = () => {
    ...
    //Service Areas
    if (query.serviceArea && query.serviceArea.length > 0) {
      let serviceAreaString = "";
      for (let i = 0; i < query.serviceArea.length; i++) {
        if (i == query.serviceArea.length - 1) {
          serviceAreaString = serviceAreaString.concat(query.serviceArea[i]);
        }
        else {
          serviceAreaString = serviceAreaString.concat(query.serviceArea[i]);
          serviceAreaString = serviceAreaString.concat(", ");
        }
      }
      filtersApplied = filtersApplied.concat({ name: 'Service Areas', lastUpdated: serviceAreaString, link: '' });
    }
    //Tags
    if (query.tagLocale != "") {
      let tagString = "";
