...
const FormPhotos = (props) => {
    ...
    const renderPhotos = () => {
        ...
        if (isLoaded && unapproved.length === 0) {
            return (
                <Box>
            ...
            if (showApprove && approved.length > 0) {
                return <Gallery photos={approved} renderImage={imageRenderer} />
            } else if (!showApprove && unapproved.length > 0) {
                return <Gallery photos={unapproved} renderImage={imageRenderer} />
            } else {
                return (
                    <Box>
                        <Text textAlign='center'>There is no approved photos for this organization.</Text>
                    </Box>
                )
            }
         }
            ...
            <Box  style={{marginLeft: '5%'}}>
                ...
                <ModalContent justifyContent='center' alignItems='center' flexDirection='column' borderRadius={4}>
                    ...
                    <ModalFooter display='flex' flexDirection='column' justifyContent='center' alignItems='center'>
                        <Box>
                            <Link color='#2F80ED' onClick={getApproved}>See approved photos</Link>
                        </Box>
                    </ModalFooter>
                </ModalContent>
            </Modal>
